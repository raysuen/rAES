...
def func_help():
    print("""
    NAME:
        rAES    --encrypt or decrypt a string 
    SYNOPSIS:
        rAES [-d|-e] -k key -s string
    DESCRIPTION:
        -d      Decryption,You can't use -d and -e at the same time.
        -e      Encrpytion,You can't use -d and -e at the same time.
        -k      Key,according to the key to encrypt or decrypt.
        -s      Spicify a string to encryption or decryption.
        ...
        while i < len(sys.argv):
            if sys.argv[i] == "-h":
                ...
                if i >= len(sys.argv):   #表示-k后面没有跟随-k的值
                    ...
                    exit(1)
                elif re.match("^-", sys.argv[i]) != None:  # 判断-k的值，如果-k的下个参数以-开头，表示没有指定-k值
                    print("The value of -k must be specified!!!")
                    exit(1)
                rkey = sys.argv[i]
            elif sys.argv[i] == "-s":
